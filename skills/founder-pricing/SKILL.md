...
2. **Package around the job, not around feature count.** One or few packages a solo founder can sell and deliver. Each package: who it is for, what is included, what is not, whether the fee is recurring, one-off, or a mix. Constraints (delivery capacity, support) from context bound how much can be sold. Where a real usage unit exists (seats, workflows run, records processed — something that actually tracks with the value the buyer gets), prefer pricing against it over a single flat fee; a flat fee that ignores how much value different buyers get leaves money on the table with the biggest ones and overcharges the smallest.

   Default toward charging more, not less, for a genuine solo-founder reason: most early pricing is set from the founder's own discomfort asking for money, not from what the buyer would actually pay, and a small change in what's charged usually moves revenue by more than an equivalent amount of extra acquisition effort would — so a stuck price is worth revisiting before a stuck channel is. Keep the package structure itself simple (one clear number and what it buys, not five tiers with crossed-out prices) — the packaging is not where the sales work should happen. And treat today's price as a hypothesis, not a permanent commitment: it is normal, and often necessary, to raise it later as evidence of value accumulates — grandfather existing customers or give advance notice rather than avoiding the conversation indefinitely.
